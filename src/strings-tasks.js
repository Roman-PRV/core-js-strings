...
function sumOfCodes(str) {
  return str
    ? str.split('').reduce((sum, elem) => +sum + +elem.charCodeAt(0), 0)
    : 0;
}
...
function extractNameFromTemplate(value) {
  return value.slice(0, -1).replace('Hello, ', '');
}
  ...
  return str
    ...
    .map((elem) => {
      return String.fromCharCode(
        /[A-Za-z]/.test(elem.at(0))
          ? elem.charCodeAt(0) + (elem.toUpperCase()[0] > 'M' ? -13 : 13)
          : elem.charCodeAt(0)
      );
    })
    .join('');
